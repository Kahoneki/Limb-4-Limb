...
SinglePlayerScene::SinglePlayerScene(sf::RenderWindow* hwnd, Input* in, SceneManager& sm, int difficulty) : sceneManager(sm), pausePopup(in), matchInvitationInterrupt(MatchInvitationInterrupt::getInstance())
{
	...
	std::cout << "Loaded single player scene\n";
}


SinglePlayerScene::~SinglePlayerScene()
{
	std::cout << "Unloading single player scene\n";
	delete players[0];
	delete players[1];
	if (itemBox != nullptr) { delete itemBox; }
	std::cout << "Unloaded single player scene\n";

}


void SinglePlayerScene::InitialiseScene() {
...
void SinglePlayerScene::InitialisePlayers() {
	//Size should be roughly in ratio 5:9
	players[0] = new Player(sf::Vector2f(56, 103), 3300.0f, 600.0f, 1350.0f, 100, 100, 0, false);
	players[1] = new AIPlayer(sf::Vector2f(56, 103), 3300.0f, 600.0f, 1350.0f, 100, 100, 0, true, difficultyLevel);
	...
	{
	case 1:
		players[1]->setFillColor(LEVEL1AICOLOUR);
		break;
	case 2:
		players[1]->setFillColor(LEVEL2AICOLOUR);
		break;
	case 3:
		players[1]->setFillColor(LEVEL3AICOLOUR);
		break;
	case 4:
		players[1]->setFillColor(LEVEL4AICOLOUR);
		break;
	case 5:
		players[1]->setFillColor(LEVEL5AICOLOUR);
		break;
	}
...
void SinglePlayerScene::InitialiseHealthBars() {
...
void SinglePlayerScene::handleInput(float dt) {
...
void SinglePlayerScene::ItemBoxCollisionCheck(Player* player) {
...
void SinglePlayerScene::PlatformCollisionCheck(Player* player) {
...
void SinglePlayerScene::AttackHitboxCheck(Player* defendingPlayer, Player* attackingPlayer) {
...
void SinglePlayerScene::ApplyKnockbackToDefendingPlayer(Player* defendingPlayer, Player* attackingPlayer, int limbIndex) {
...
void SinglePlayerScene::ApplyKnockbackToAttackingPlayer(Player* defendingPlayer, Player* attackingPlayer, int limbIndex) {
...
void SinglePlayerScene::HealthBarUpdate() {
...
void SinglePlayerScene::DebugRender() {
	for (Player* player : players) {
